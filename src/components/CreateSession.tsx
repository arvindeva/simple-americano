...
export default function CreateSession() {
  ...
  const addPlayerToList = () => {
    if (sessionState.newPlayerName.trim() && !sessionState.playerNames.includes(sessionState.newPlayerName.trim())) {
      updateSessionState({
        playerNames: [...sessionState.playerNames, sessionState.newPlayerName.trim()],
        newPlayerName: ''
      });
    }
  };
  ...
  const removePlayerFromList = (playerName: string) => {
    updateSessionState({
      playerNames: sessionState.playerNames.filter(name => name !== playerName)
    });
  };
    ...
    switch (sessionState.currentStep) {
          ...
          <Card className="w-full">
            <CardHeader className="text-center p-4 sm:p-6 relative">
              <div className="absolute top-4 left-4">
                <Button variant="ghost" size="icon" onClick={returnToPreviousStep} className="shrink-0">
                  <ArrowLeft className="h-4 w-4" />
                </Button>
              </div>
              <CardTitle className="text-lg sm:text-xl">Tournament Name</CardTitle>
              <CardDescription className="text-sm sm:text-base">Give your tournament a name</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 p-4 sm:p-6">
              <Input
                placeholder="Enter tournament name"
                value={sessionState.tournamentName}
                onChange={(e) => updateSessionState({ tournamentName: e.target.value })}
              ...
              <div className="flex flex-col gap-2">
                ...
                <Button onClick={returnToPreviousStep} variant="ghost" className="w-full">
                  Cancel
                </Button>
              </div>
            </CardContent>
        ...
        return (
          <Card className="w-full">
            <CardHeader className="text-center p-4 sm:p-6">
              <CardTitle className="text-lg sm:text-xl">How many fields?</CardTitle>
              <CardDescription className="text-sm sm:text-base">Select the number of padel courts available</CardDescription>
            </CardHeader>
            <CardContent className="p-4 sm:p-6">
              ...
              <Button onClick={returnToPreviousStep} variant="ghost" className="w-full mt-4">
                Back
              </Button>
            </CardContent>
          </Card>
      ...
      case 3:
        return (
          <Card className="w-full">
            <CardHeader className="text-center p-4 sm:p-6">
              <CardTitle className="text-lg sm:text-xl">Points per game?</CardTitle>
              <CardDescription className="text-sm sm:text-base">Select the target score for each match</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 p-4 sm:p-6">
      ...
      case 4:
        return (
          <Card className="w-full">
            <CardHeader className="text-center p-4 sm:p-6">
              <CardTitle className="text-lg sm:text-xl">Add Players</CardTitle>
              <CardDescription className="text-sm sm:text-base">Enter player names (minimum 4 required)</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 p-4 sm:p-6">
              <div className="flex gap-2">
                <Input
                  ...
                  onChange={(e) => updateSessionState({ newPlayerName: e.target.value })}
              ...
              {sessionState.playerNames.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium text-sm sm:text-base">Players ({sessionState.playerNames.length}):</h4>
                  <div className="space-y-1 max-h-32 sm:max-h-40 overflow-y-auto">
                    {sessionState.playerNames.map((playerName, index) => (
                      <div key={index} className="flex items-center justify-between bg-muted p-2 sm:p-3 rounded">
                        <span className="text-xs sm:text-sm truncate flex-1 mr-2">{playerName}</span>
                        <Button
              ...
              <div className="flex flex-col gap-2 pt-2">
                ...
                <Button onClick={returnToPreviousStep} variant="ghost" className="w-full h-10">
                  Back
                </Button>
              </div>
            </CardContent>
